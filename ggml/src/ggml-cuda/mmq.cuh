...
#define CASE(type_, nthreads_, occupancy_, I_, J_, sram_layout_, K_vram_, stream_k_, fallback_)                                           \
    if ((type == (type_) || ((type == GGML_TYPE_ZNQ2 || type == GGML_TYPE_ZNQ3 || type == GGML_TYPE_ZNQ4) && (type_) == GGML_TYPE_IQ4_NL)) && J == (J_) && fallback == (fallback_)) {                                                                        \
...
static constexpr __host__ __device__ tile_x_sizes mmq_get_dp4a_tile_x_sizes(ggml_type type, int I) {
    switch (type) {
        ...
        case GGML_TYPE_ZNQ2:    return MMQ_DP4A_TXS_Q8_0;
        case GGML_TYPE_ZNQ3:    return MMQ_DP4A_TXS_Q8_0;
        case GGML_TYPE_ZNQ4:    return MMQ_DP4A_TXS_Q8_0;
...
// ---------------------------------------------------------------------------------------------
            ...
            case GGML_TYPE_ZNQ2:
                return ggml_cuda_mmq_util_funcs(
                    4,
                    ggml_cuda_mmq_load_tiles_znq<type, J, fallback, 2>,
                    ggml_cuda_mmq_vec_dot_q8_0_q8_1_dp4a<type, J, fallback>,
                    ggml_cuda_mmq_write_back_dp4a<type, J, fallback>);
            case GGML_TYPE_ZNQ3:
                return ggml_cuda_mmq_util_funcs(
                    4,
                    ggml_cuda_mmq_load_tiles_znq<type, J, fallback, 3>,
                    ggml_cuda_mmq_vec_dot_q8_0_q8_1_dp4a<type, J, fallback>,
                    ggml_cuda_mmq_write_back_dp4a<type, J, fallback>);
            case GGML_TYPE_ZNQ4:
                return ggml_cuda_mmq_util_funcs(
                    4,
                    ggml_cuda_mmq_load_tiles_znq<type, J, fallback, 4>,
                    ggml_cuda_mmq_vec_dot_q8_0_q8_1_dp4a<type, J, fallback>,
                    ggml_cuda_mmq_write_back_dp4a<type, J, fallback>);
            case GGML_TYPE_IQ4_NL:
                return ggml_cuda_mmq_util_funcs(
...
static __global__ void mul_mat_q(
        ...
        const uint3 ntx, const int32_t * __restrict__ block_expert, const int32_t * __restrict__ block_start, const int n_experts) {
    ...
    if constexpr (!ggml_cuda_mmq_get_stream_k(type, J, fallback)) {
        ...
        // Routed MMQ uses a compact block map built once per dispatch. blockIdx.y then
        // names a real expert/column tile directly instead of requiring every workgroup
        // to scan all expert bounds. Padding blocks in the host-known upper bound exit.
        if (block_expert) {
...
// Build a compact mapping from MMQ column-block index to expert. expert_bounds is the
// prefix sum generated by the routed MMID helper. The actual number of blocks is only
// known on device, so block_start[n_experts] acts as the device-side real grid bound.
static __global__ void mmq_build_routed_block_map(
        const int32_t * __restrict__ expert_bounds, const int n_experts, const int tile_cols,
        int32_t * __restrict__ block_start, int32_t * __restrict__ block_expert) {
    extern __shared__ int32_t start_shared[];
    const int tid = threadIdx.x;

    for (int expert = tid; expert < n_experts; expert += blockDim.x) {
        const int rows = expert_bounds[expert + 1] - expert_bounds[expert];
        start_shared[expert] = (rows + tile_cols - 1) / tile_cols;
    }
    __syncthreads();
    ...
    if (tid == 0) {
        ...
        for (int expert = 0; expert < n_experts; ++expert) {
            const int tiles = start_shared[expert];
            start_shared[expert] = total;
            total += tiles;
        }
        start_shared[n_experts] = total;
    }
    __syncthreads();

    for (int expert = tid; expert <= n_experts; expert += blockDim.x) {
        block_start[expert] = start_shared[expert];
    }

    const int total = start_shared[n_experts];
    for (int block = tid; block < total; block += blockDim.x) {
        int lo = 0;
        int hi = n_experts;
        while (lo < hi) {
            const int mid = (lo + hi) >> 1;
            if (start_shared[mid] <= block) {
                lo = mid + 1;
            } else {
                hi = mid;
            }
        }
        block_expert[block] = lo - 1;
    }
}
...
static void launch_mul_mat_q(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    const bool compact_routed_tiles = args.ids_dst != nullptr && !ggml_cuda_mmq_get_stream_k(type, J, fallback, cc);
    const int ntx = compact_routed_tiles
        ? (args.ncols_y + config.J - 1) / config.J + args.nchannels_y - 1
        : (args.ncols_max + config.J - 1) / config.J;
    const int ntzw = compact_routed_tiles ? args.nsamples_y : args.nchannels_y * args.nsamples_y;
    ...
    if (!ggml_cuda_mmq_get_stream_k(type, J, fallback, cc)) {
        ggml_cuda_pool_alloc<int32_t> block_start(ctx.pool(id));
        ggml_cuda_pool_alloc<int32_t> block_expert(ctx.pool(id));
        int n_experts = 0;

        if (compact_routed_tiles) {
            n_experts = args.nchannels_y;
            block_start.alloc(n_experts + 1);
            block_expert.alloc(ntx);

            constexpr int build_nthreads = 256;
            mmq_build_routed_block_map<<<1, build_nthreads, (n_experts + 1)*sizeof(int32_t), stream>>>(
                args.expert_bounds, n_experts, config.J, block_start.ptr, block_expert.ptr);
        ...
        mul_mat_q<type, J, fallback><<<block_nums_xy_tiling, block_dims, nbytes_shared, stream>>>
            (args.x, args.y, args.ids_dst, args.expert_bounds, args.dst, nullptr, args.y_scale,
             ...
             ntx_fd, block_expert.ptr, block_start.ptr, n_experts);
        return;
    }
...
extern DECL_MMQ_CASE(GGML_TYPE_ZNQ2);
extern DECL_MMQ_CASE(GGML_TYPE_ZNQ3);
extern DECL_MMQ_CASE(GGML_TYPE_ZNQ4);
