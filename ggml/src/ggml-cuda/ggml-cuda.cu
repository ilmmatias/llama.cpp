...
#include "ggml-cuda/qsa-block-score.cuh"
...
static bool ggml_cuda_should_fuse_mul_mat_vec_q(const ggml_tensor * tensor, bool with_gate = true) {
    ...
    const bool supported_src1 = src1->type == GGML_TYPE_F32 || src1->type == GGML_TYPE_Q8_1;
    bool use_mul_mat_vec_q = ggml_is_quantized(src0->type) && !bad_padding_clear && supported_src1 &&
...
static bool ggml_cuda_mul_mat_id_needs_sync(const ggml_tensor * dst, const int cc) {
    ...
    const bool src1_q8  = src1->type == GGML_TYPE_Q8_1;
    if ((!src1_f32 && !src1_q8) || dst->type != GGML_TYPE_F32) {
        return true;
    }
    ...
    if (dst->ne[2] <= MMVQ_MAX_BATCH_SIZE) {
        ...
        } else if (src1_f32 && GGML_CUDA_CC_IS_AMD(cc)) {
            return false;
        }
    }

    // Pre-quantized Q8_1 is intentionally an MMVQ-only input. MMQ uses a
    // different Q8_1 packing, so it cannot consume this buffer directly.
    if (src1_q8) {
        return true;
    }
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if ((src1->type == GGML_TYPE_F32 || src1->type == GGML_TYPE_Q8_1) && dst->type == GGML_TYPE_F32) {
        ...
        // Q8_1 input is a fast-path-only representation used to skip MMVQ's
        // F32 quantization launch. If it did not qualify for MMVQ above, do
        // not feed it to MMQ/MMF, which expect their normal input layouts.
        if (src1->type == GGML_TYPE_Q8_1) {
            GGML_ABORT("pre-quantized Q8_1 MUL_MAT_ID requires MMVQ");
        }
        ...
        for (int64_t i12 = 0; i12 < ne12; ++i12) { // tokens
            for (int64_t iex = 0; iex < n_expert_used; ++iex) {
                ...
                if (expert_to_use == i02) {
                    // A token may route multiple slots to the same expert (for example compact hash routing).
                    // Keep every route slot; collapsing at the first match would drop duplicate contributions.
...
static int ggml_cuda_try_qsa_mask_fusion(ggml_backend_cuda_context & ctx, ggml_cgraph * graph, int i) {
    ...
    // Keep stream transitions and fork/join synchronization outside the skipped span.
    for (const auto & [fork, event] : ctx.stream_context().concurrent_events) {
        const auto first = event.stream_mapping.find(zeros);
        const int stream = first == event.stream_mapping.end() ? 0 : first->second;
        for (int j = i; j <= output; ++j) {
            const ggml_tensor * node = graph->nodes[j];
            const auto mapped = event.stream_mapping.find(node);
            if ((j < output && node == fork) || (j > i && node == event.join_node) ||
    ...
    switch (op->op) {
        ...
        case GGML_OP_QSA_BLOCK_SCORE:
            return op->src[0]->type == GGML_TYPE_F32 && op->src[1]->type == GGML_TYPE_F32 &&
                ...
                op->type == GGML_TYPE_F32 &&
                ggml_is_contiguous_rows(op->src[0]) && ggml_is_contiguous_rows(op->src[1]) &&
